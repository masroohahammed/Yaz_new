...
<form method="post" action="<?= $isEdit ? base_url('payments/'.$payment['id'].'/update') : base_url('payments') ?>"><?= csrf_field() ?>
  <div class="row g-3">
    ...
    <div class="col-md-4"><label class="form-label">Payment method</label><select name="payment_method" class="form-select"><?php helper('fm'); foreach (fm_payment_methods('payment') as $m => $mLabel): ?><option value="<?= $m ?>" <?= old('payment_method',$payment['payment_method']??'cash')===$m?'selected':'' ?>><?= esc($mLabel) ?></option><?php endforeach; ?></select></div>
