...
<div class="page-header">
  <div>
    ...
    <div class="small text-muted">Audit trail — who did what, when, and from which IP (super admin)</div>
  </div>
  <div class="d-flex gap-2">
    <a href="<?= base_url('reports/export/activity/csv?from=' . urlencode($from ?? '') . '&to=' . urlencode($to ?? '')) ?>" class="btn btn-fm-outline btn-sm"><i class="bi bi-filetype-csv me-1"></i>CSV</a>
    ...
    <form method="get" class="row g-2 align-items-end">
      ...
      <div class="col-md-2">
        ...
        <select name="module" class="form-select form-select-sm">
          ...
          <?php foreach (['work_orders','job_cards','helpdesk','finance','procurement','purchase_orders','purchase_requests','settings','users','facilities','inventory','vendors'] as $m): ?>
      ...
      <div class="col-md-2">
        ...
        <input type="text" name="action" class="form-control form-control-sm" value="<?= esc($filterAction ?? '') ?>" placeholder="e.g. complete">
      </div>
      <div class="col-md-2">
      ...
      <div class="col-md-1">
        <a href="<?= base_url('reports/activity-log') ?>" class="btn btn-fm-outline btn-sm w-100">Reset</a>
      </div>
    </form>
  </div>
</div>
    ...
    <div class="table-responsive">
      <table class="fm-table table-sm">
        ...
        <tr>
          ...
          <td class="small">
            <?php if ($l['record_id'] && ($l['module'] ?? '') === 'work_orders'): ?>
              <a href="<?= base_url('workorders/view/' . (int)$l['record_id']) ?>">#<?= (int)$l['record_id'] ?></a>
            <?php elseif ($l['record_id']): ?>
              #<?= (int)$l['record_id'] ?>
            <?php else: ?>—<?php endif; ?>
          </td>
          <td class="small text-muted" style="max-width:360px"><?= esc($l['description'] ?? '') ?></td>
          <td class="x-small text-muted text-nowrap"><?= esc($l['ip_address'] ?? '') ?></td>
        </tr>
        <?php endforeach; ?>
        <?php if (empty($logs)): ?>
        <tr><td colspan="7" class="text-center py-4 text-muted">No activity logged for this period.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div>
    ...
    <nav>
      <ul class="pagination pagination-sm mb-0 justify-content-center">
        ...
        <li class="page-item <?= $i === $currentPage ? 'active' : '' ?>">
          <a class="page-link" href="?page=<?= $i ?>&from=<?= esc($from ?? '') ?>&to=<?= esc($to ?? '') ?>&module=<?= esc($filterModule ?? '') ?>&action=<?= esc($filterAction ?? '') ?>&user_id=<?= (int)($filterUser ?? 0) ?>"><?= $i ?></a>
        </li>
        <?php endfor; ?>
      </ul>
    </nav>
