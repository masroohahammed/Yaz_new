...
function endpoints(): array
{
    ...
    return [
        ...
        [
            ...
            'method' => 'GET', 'path' => '/api/v1/employee/profile', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Get employee master profile: code, department, designation, shift times, facility.',
            'curl' => "curl -X GET \"{$base}/employee/profile\" \\\n  {$authHeader} \\\n  -H \"Accept: application/json\"",
            'request' => null,
            'response' => [
                'status' => true,
                'data' => [
                    'employee_id' => 12, 'emp_code' => 'EMP-001', 'name' => 'Sara Ali',
                    'email' => 'sara@example.com', 'department' => 'FM', 'designation' => 'Technician',
                    'shift_start' => '08:00:00', 'shift_end' => '17:00:00', 'facility_name' => 'Yazwa Tower',
                ],
            ],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'GET', 'path' => '/api/v1/employee/dashboard', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Employee home dashboard: today\'s attendance, break status, monthly KPIs, pending leaves.',
            'curl' => "curl -X GET \"{$base}/employee/dashboard\" \\\n  {$authHeader}",
            'request' => null,
            'response' => [
                'status' => true,
                'data' => [
                    'employee' => ['emp_code' => 'EMP-001', 'name' => 'Sara Ali'],
                    'today' => ['check_in' => '08:02:00', 'status' => 'present'],
                    'on_break' => false,
                    'month_summary' => ['present_days' => 18, 'late_days' => 1, 'total_hours' => 142.5],
                    'pending_leaves' => 1, 'is_manager' => false,
                ],
            ],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'POST', 'path' => '/api/v1/employee/attendance/check-in', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Record daily check-in. Optional GPS coordinates. Marks late if after shift start + 15 minutes.',
            'curl' => "curl -X POST \"{$base}/employee/attendance/check-in\" \\\n  {$authHeader} \\\n  {$jsonHeader} \\\n  -d '{\"latitude\":25.2854,\"longitude\":51.5310}'",
            'request' => ['latitude' => 25.2854, 'longitude' => 51.531],
            'response' => ['status' => true, 'message' => 'Checked in at 08:02', 'data' => ['status' => 'present', 'check_in' => '2026-09-11 08:02:00']],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'POST', 'path' => '/api/v1/employee/attendance/check-out', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Record daily check-out. Calculates hours_worked and overtime_hrs automatically.',
            'curl' => "curl -X POST \"{$base}/employee/attendance/check-out\" \\\n  {$authHeader} \\\n  {$jsonHeader} \\\n  -d '{\"latitude\":25.2854,\"longitude\":51.5310}'",
            'request' => ['latitude' => 25.2854, 'longitude' => 51.531],
            'response' => ['status' => true, 'message' => 'Checked out', 'data' => ['check_out' => '2026-09-11 17:05:00', 'hours_worked' => 9.05, 'overtime_hrs' => 0.08]],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'POST', 'path' => '/api/v1/employee/attendance/break-start', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Start a break period for today.',
            'curl' => "curl -X POST \"{$base}/employee/attendance/break-start\" \\\n  {$authHeader} \\\n  {$jsonHeader} \\\n  -d '{}'",
            'request' => null,
            'response' => ['status' => true, 'message' => 'Break started', 'data' => ['break_start' => '2026-09-11 12:00:00']],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'POST', 'path' => '/api/v1/employee/attendance/break-end', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'End the current open break and return duration in minutes.',
            'curl' => "curl -X POST \"{$base}/employee/attendance/break-end\" \\\n  {$authHeader} \\\n  {$jsonHeader} \\\n  -d '{}'",
            'request' => null,
            'response' => ['status' => true, 'message' => 'Break ended', 'data' => ['duration_minutes' => 45]],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'GET', 'path' => '/api/v1/employee/attendance?month=YYYY-MM', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Get monthly attendance records for the employee.',
            'curl' => "curl -X GET \"{$base}/employee/attendance?month=2026-09\" \\\n  {$authHeader}",
            'request' => null,
            'response' => [
                'status' => true,
                'data' => [['date' => '2026-09-01', 'status' => 'present', 'check_in' => '08:00:00', 'check_out' => '17:00:00', 'hours_worked' => 9.0]],
                'count' => 1,
            ],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'GET', 'path' => '/api/v1/employee/leaves', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'List own leave requests and their approval status.',
            'curl' => "curl -X GET \"{$base}/employee/leaves\" \\\n  {$authHeader}",
            'request' => null,
            'response' => ['status' => true, 'data' => [['id' => 5, 'leave_type' => 'annual', 'from_date' => '2026-09-20', 'to_date' => '2026-09-22', 'status' => 'pending']]],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'POST', 'path' => '/api/v1/employee/leaves', 'auth' => 'Bearer JWT (linked employee)',
            'desc' => 'Submit a new leave request for approval.',
            'curl' => "curl -X POST \"{$base}/employee/leaves\" \\\n  {$authHeader} \\\n  {$jsonHeader} \\\n  -d '{\"leave_type\":\"annual\",\"from_date\":\"2026-09-20\",\"to_date\":\"2026-09-22\",\"reason\":\"Family travel\"}'",
            'request' => ['leave_type' => 'annual', 'from_date' => '2026-09-20', 'to_date' => '2026-09-22', 'reason' => 'Family travel'],
            'response' => ['status' => true, 'message' => 'Leave request submitted', 'id' => 6],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'GET', 'path' => '/api/v1/employee/team-attendance?date=YYYY-MM-DD', 'auth' => 'Bearer JWT (supervisor/manager)',
            'desc' => 'Supervisor/Manager: view team attendance for a specific date.',
            'curl' => "curl -X GET \"{$base}/employee/team-attendance?date=2026-09-11\" \\\n  {$authHeader}",
            'request' => null,
            'response' => ['status' => true, 'data' => [['employee_id' => 12, 'name' => 'Sara Ali', 'status' => 'present', 'check_in' => '08:02:00']]],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'GET', 'path' => '/api/v1/employee/leave-approvals', 'auth' => 'Bearer JWT (supervisor/manager)',
            'desc' => 'Supervisor/Manager: list pending leave requests awaiting approval.',
            'curl' => "curl -X GET \"{$base}/employee/leave-approvals\" \\\n  {$authHeader}",
            'request' => null,
            'response' => ['status' => true, 'data' => [['id' => 5, 'employee_name' => 'Sara Ali', 'leave_type' => 'annual', 'from_date' => '2026-09-20', 'status' => 'pending']]],
        ],
        [
            'group' => 'Employee Self-Service',
            'method' => 'POST', 'path' => '/api/v1/employee/leaves/{id}/action', 'auth' => 'Bearer JWT (supervisor/manager)',
            'desc' => 'Supervisor/Manager: approve or reject a leave request. Approved leaves mark attendance as leave.',
            'curl' => "curl -X POST \"{$base}/employee/leaves/5/action\" \\\n  {$authHeader} \\\n  {$jsonHeader} \\\n  -d '{\"action\":\"approve\"}'",
            'request' => ['action' => 'approve', 'reason' => 'Required when action is reject'],
            'response' => ['status' => true, 'message' => 'Leave approved'],
        ],
